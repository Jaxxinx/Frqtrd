...
class MultiOffsetLamboV0(IStrategy):
    ...
    def custom_stoploss(self, pair: str, trade: 'Trade', current_time: 'datetime',
        ...
        """
        Dynamic stoploss: tightens as profit grows.
        Base SL: -10% (down from -50%)
        After +1.5% profit: move to breakeven (0%)
        After +2.5% profit: lock +1%
        After +4% profit: lock +2.5%
        After +6% profit: lock +4%
        """
        if current_profit >= 0.06:
            return -0.04   # lock in 4% profit
        if current_profit >= 0.04:
            return -0.025  # lock in 2.5% profit
        if current_profit >= 0.025:
            return -0.01   # lock in 1% profit
        if current_profit >= 0.015:
            return 0.0     # breakeven
        return self.stoploss  # -10% baseline
